...
function Product({ scrollRef }) {
  ...
  return (
    <RoundedBox ref={mesh} args={[2.4, 1.2, 0.28]} radius={0.08} smoothness={8}>
...
function Scene({ scrollRef }) {
  return (
    <Canvas camera={{ position: [0, 0, 5], fov: 42 }} gl={{ antialias: true, alpha: true }}>
      ...
      <directionalLight position={[4, 6, 5]} intensity={5} />
      <directionalLight position={[-4, -2, -3]} intensity={2} color="#818cf8" />
      <pointLight position={[0, 4, 3]} intensity={8} color="#c7d2fe" distance={10} />
...
function ChapterText({ chapter, index, chapterIndex, isMobile }) {
  ...
  return (
    <motion.div style={{
      ...
      <p style={{ fontSize: isMobile ? 13 : 15, lineHeight: 1.7, color: 'rgba(255,255,255,0.5)' }}>
        {chapter.desc}
      </p>
    </motion.div>
  )
...
function ProgressDots({ scrollYProgress, isMobile }) {
  ...
  return (
    <div id="showcase2" ref={containerRef} style={{ height: '400vh', background: '#030308' }}>
      <div style={{ position: 'sticky', top: 0, height: '100vh', overflow: 'hidden' }}>
        ...
        <ProgressDots scrollYProgress={scrollYProgress} isMobile={isMobile} />
        <motion.p
